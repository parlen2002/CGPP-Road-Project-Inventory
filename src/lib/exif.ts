/* ─────────────────────────────────────────────────────────────
   EXIF GPS READER — extracts the capture position from a
   geotagged JPEG entirely in-browser (no uploads, no API keys).
   Parses APP1 → TIFF → GPS IFD (tags 1–4). Returns null when the
   file has no GPS metadata.
   ────────────────────────────────────────────────────────────── */
...
function parseTiff(dv: DataView, t0: number): [number, number] | null {
  ...
  for (let i = 0; i < n; i++) {
    const e = ifd0 + 2 + i * 12;
    if (u16(e) === 0x8825) gpsPtr = u32(e + 8); // GPSInfo pointer
  }
...
/** three RATIONAL values (deg, min, sec) at a GPS tag entry */
