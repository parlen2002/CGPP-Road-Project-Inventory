/* Vector format readers — KML/GPX centerlines, SHP/DBF lots, GeoJSON. */
...
export function parseKML(text: string): LineFeature[] {
  ...
  if (doc.querySelector("parsererror")) throw new Error("KML is malformed.");
...
export function parseGeoJSON(text: string): { lots: LotFeature[]; lines: LineFeature[] } {
  ...
  feats.forEach((f, i) => {
    ...
    if (g.type === "Polygon") lots.push({ ring: g.coordinates[0].map((p: number[]): LatLng => [p[1], p[0]]), props });
    else if (g.type === "MultiPolygon") g.coordinates.forEach((poly: number[][][]) => lots.push({ ring: poly[0].map((p: number[]): LatLng => [p[1], p[0]]), props }));
    else if (g.type === "LineString") lines.push({ name, line: g.coordinates.map((p: number[]): LatLng => [p[1], p[0]]) });
    else if (g.type === "MultiLineString") g.coordinates.forEach((l: number[][], j: number) => lines.push({ name: `${name} (${j + 1})`, line: l.map((p: number[]): LatLng => [p[1], p[0]]) }));
  });
  if (!lots.length && !lines.length) throw new Error("No polygon or linestring features found.");
  return { lots, lines };
}

export function parseSHP(buffer: ArrayBuffer): LatLng[][] {
  const dv = new DataView(buffer);
  if (dv.getUint32(0, false) !== 9994) throw new Error("Not a valid shapefile.");
  const shapeType = dv.getInt32(32, true);
  if (shapeType !== 5 && shapeType !== 15 && shapeType !== 0) throw new Error("Only polygon shapefiles are supported.");
  ...
  while (pos + 12 <= fileLen && pos + 12 <= buffer.byteLength) {
    ...
    if (st === 5 || st === 15) {
      ...
      for (let i = 0; i < numPoints; i++) pts.push([dv.getFloat64(ptStart + i * 16 + 8, true), dv.getFloat64(ptStart + i * 16, true)]);
  ...
  if (!rings.length) throw new Error("No polygon records decoded.");
  return rings;
}
