/* Cadastre + centerline model and the ROW impact engine
   (clipping a lot against a flat-sided corridor buffer). */
...
/* deterministic sample cadastre around the poblacion / project corridors */
...
export function runROWAnalysis(parcels: Parcel[], cl: Centerline): ROWResult {
  ...
  const ringArea = polygonAreaM2(corridorRing);
  return { rows, corridorAreaM2: Math.round(ringArea), corridorRing, govM2, privM2, totalCost };
}
