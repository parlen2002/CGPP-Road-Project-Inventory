/* Geospatial console map — OpenStreetMap / Esri (no API keys).
   Draws only what belongs to the project ledger: linked roads, centerline
   axes and station pins resolved geotag → centerline → barangay centroid. */
...
import { statusOf, typeAbbr, fmtPesoM, projectPoint } from "../data/registry";
import { useStore, type Snapshot } from "../state/store";
...
/** pin resolution shared with popups — geotag → linked centerline → barangay */
function pinInfo(s: Snapshot, p: Snapshot["records"][number]) {
  const geo = (p.attachments ?? []).find((a) => a.lat != null && a.lng != null);
  if (geo) return { pt: [geo.lat!, geo.lng!] as [number, number], kind: "geotag" as const };
  const cl = s.centerlines.find((c) => c.projectId === p.id);
  if (cl && cl.line.length) {
    return {
      pt: [
        cl.line.reduce((sum, x) => sum + x[0], 0) / cl.line.length,
        cl.line.reduce((sum, x) => sum + x[1], 0) / cl.line.length,
      ] as [number, number],
      kind: "centerline" as const,
    };
  }
  return { pt: projectPoint(p), kind: "barangay" as const };
}
...
export default function MapView({ roads, focus, onLocate }: {
  ...
  const s = useStore();
  const { records, barangays, centerlines } = s;
    ...
    <div className="relative overflow-hidden rounded-[4px] border-2 border-ink-800 bg-ink-950">
      <MapContainer center={[9.748, 118.745]} zoom={13} className="h-[540px] w-full" scrollWheelZoom zoomControl={false}>
        ...
        {layers.roads && linkedRoads.map((r) => {
          const cm = classMeta[r.roadClass];
          const cond = conditionMeta[r.condition];
          return (
            <span key={r.id}>
              <Polyline positions={r.geometry} pathOptions={{ color: "#0c1913", weight: cm.weight + 3, opacity: 0.5 }} interactive={false} />
              <Polyline positions={r.geometry} eventHandlers={{ click: () => onLocate?.(r) }}
                pathOptions={{ color: cond.color, weight: cm.weight, opacity: 0.95 }}>
                <Tooltip className="rpis-tip" direction="top" offset={[0, -6]}>
                  {r.roadClass.toUpperCase()} · {r.name} — {r.condition} · PCI {r.pci}
                </Tooltip>
              </Polyline>
            </span>
          );
        ...
        {layers.pins && records.map((p) => {
          ...
          const { pt, kind } = pinInfo(s, p);
          return (
            <CircleMarker key={p.id} center={pt} radius={8}
              pathOptions={{ color: meta.color, weight: 2.5, fillColor: meta.color, fillOpacity: 0.28, dashArray: kind === "barangay" ? "2 3" : undefined }}>
              <Popup className="rpis-popup">
                <div style={{ minWidth: 220 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 10px", marginTop: 10, fontFamily: "var(--font-mono)", fontSize: 11 }}>
                    <span style={{ color: "#7d9183" }}>BARANGAYS</span><span style={{ color: "#ffc24d", textAlign: "right" }}>{p.location.barangays.join(", ")}</span>
                    ...
                    <span style={{ color: "#7d9183" }}>IN-CHARGE</span>
                    <span style={{ color: "#ffc24d", textAlign: "right" }}>{s.personnel.find((x) => x.id === p.inchargeId)?.name ?? "—"}</span>
                  </div>
                  ...
                  <p style={{ fontFamily: "var(--font-mono)", fontSize: 9.5, color: "#7d9183", marginTop: 10, letterSpacing: "0.06em" }}>
                    {kind === "geotag" && <>PINNED VIA GEOTAGGED IMAGE — EXIF GPS</>}
                    {kind === "centerline" && <>PINNED VIA LINKED CENTERLINE — LOT &amp; ROW</>}
                    {kind === "barangay" && <>PIN AT BARANGAY CENTROID — NO FIELD CAPTURE YET</>}
