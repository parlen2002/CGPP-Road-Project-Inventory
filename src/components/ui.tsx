...
export const surfaceMeta: Record<Surface, string> = {
  Concrete: "#CBD4C2", Asphalt: "#9DB8A6", Gravel: "#F0A32B", Earth: "#DE5A36",
};

export const fmtNum = (n: number, d = 1) => n.toLocaleString("en-PH", { minimumFractionDigits: d, maximumFractionDigits: d });
      ...
      <div className="flex items-end justify-between gap-6 border-2 border-ink-800 bg-paper-100">
        ...
        <div className="hidden shrink-0 border-l-2 border-ink-800 md:block">
          <div className="grid grid-cols-3">
            {cells.map(([k, v], i) => (
              <div key={k} className={`min-w-[104px] px-3 py-2 ${i % 3 !== 2 ? "border-r border-line-400" : ""} ${i < 3 ? "border-b border-line-400" : ""}`}>
...
export function StatusPill({ label, color, soft }: { label: string; color: string; soft: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-[3px] px-2 py-0.5 font-mono text-[10px] font-semibold tracking-wider uppercase"
