...
function BarangayForm({ editing, previewId, sourceList, onClose }: {
  ...
  const [f, setF] = useState(() => editing
    ...
    : { psgc: "1731500068", name: "", dataSource: sourceList[0] ?? DATA_SOURCES[0], lat: "9.7400", lng: "118.7400", population: "0" });
        ...
        <div className="grid grid-cols-2 gap-3 p-5">
          ...
          <div>
            ...
            <select className={inputCls} value={f.dataSource} onChange={(e) => setF({ ...f, dataSource: e.target.value })}>
              {sourceList.map((d) => <option key={d} value={d}>{d}</option>)}
            </select>
          </div>
...
export default function BarangayRegistry({ onLocate }: { onLocate: (p: [number, number], zoom?: number) => void }) {
  const { barangays, records, catalogItems, admin } = useStore();
  const [search, setSearch] = useState("");
  const [modal, setModal] = useState<null | { editing: Barangay | null }>(null);
  const [del, setDel] = useState<Barangay | null>(null);

  const sourceList = (() => { const o = optionsOf(catalogItems, "brgyDataSources"); return o.length ? o : [...DATA_SOURCES]; })();
  ...
  return (
    <div className="anim-fade-in rounded-[4px] border-2 border-ink-800 bg-paper-100">
                ...
                <tr key={b.id} className="group transition-colors hover:bg-ink-900/[0.045]">
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex justify-end gap-1">
                      ...
                      <button onClick={() => (admin ? setDel(b) : toast("Program-Admin mode required", "info", "toggle ADMIN on the top bar to remove registry entries"))}
      ...
      {modal && <BarangayForm editing={modal.editing} previewId={modal.editing?.id ?? nextKey} sourceList={sourceList} onClose={() => setModal(null)} />}
