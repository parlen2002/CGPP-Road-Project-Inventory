/* Barangay registry — Ten-Digit PSGC code, name, data source, centroid,
   population. Encode / edit / remove; renames propagate to project records. */

import { useMemo, useState } from "react";
import { createPortal } from "react-dom";
import { optionsOf } from "../data/catalogs";
import { DATA_SOURCES, type Barangay } from "../data/barangays";
import { useStore, addBarangay, updateBarangay, renameBarangay, deleteBarangay } from "../state/store";
      ...
      <div className="anim-fade-up relative my-6 w-full max-w-lg rounded-[4px] border-2 border-ink-800 bg-paper-100 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-3 p-5">
          ...
          <div>
            ...
            <CatalogSelect group="brgyDataSources" fallback={[...DATA_SOURCES]} value={f.dataSource} onChange={(v) => setF({ ...f, dataSource: v })} />
          </div>
          ...
          <div><label className={labelCls}>Centroid Latitude</label>
            <input className={inputCls} value={f.lat} onChange={(e) => setF({ ...f, lat: e.target.value })} inputMode="decimal" placeholder="9.7400" /></div>
          <div><label className={labelCls}>Centroid Longitude</label>
            <input className={inputCls} value={f.lng} onChange={(e) => setF({ ...f, lng: e.target.value })} inputMode="decimal" placeholder="118.7400" /></div>
          <div className="col-span-2"><label className={labelCls}>Population (2024 POPCEN)</label>
            <input className={inputCls} value={f.population} onChange={(e) => setF({ ...f, population: e.target.value })} inputMode="numeric" placeholder="0" /></div>
        </div>
        <div className="flex items-center justify-between border-t border-line-300 bg-paper-200 px-5 py-3.5">
          <p className="font-mono text-[9.5px] text-text-400 uppercase">{editing ? "primary key is immutable" : `new key ${previewId} will be issued`}</p>
...
export default function BarangayRegistry({ onLocate }: { onLocate: (p: [number, number], zoom?: number) => void }) {
  ...
  const nextKey = `BRGY-${String(barangays.length + 1).padStart(3, "0")}`;
  ...
  return (
    <div className="anim-fade-in rounded-[4px] border-2 border-ink-800 bg-paper-100">
      <div className="relative flex flex-wrap items-center gap-2 border-b-2 border-ink-800 bg-paper-200 p-3">
        ...
        <p className="font-mono text-[9.5px] tracking-[0.14em] text-text-400 uppercase">{filtered.length} / {barangays.length} barangays</p>
        {can.create && (
        ...
        <table className="w-full min-w-[960px] border-collapse">
          <thead className="bg-ink-900 text-paper-300">
            <tr className="[&>th]:sticky [&>th]:top-0 [&>th]:z-10 [&>th]:bg-ink-900 [&>th]:border-b-2 [&>th]:border-amber-500/70 [&>th]:px-3 [&>th]:py-2.5 [&>th]:text-left [&>th]:font-mono [&>th]:text-[9.5px] [&>th]:font-semibold [&>th]:tracking-[0.14em] [&>th]:uppercase">
              <th>Key</th><th>Ten-Digit PSGC Code</th><th>Barangay Name</th><th>Data Source</th>
              <th className="!text-right">Pop. 2024</th><th>Centroid</th><th className="!text-right">Projects</th><th className="!text-right">Actions</th>
            </tr>
          </thead>
            ...
            {filtered.map((b) => {
              ...
              return (
                <tr key={b.id} className="group transition-colors hover:bg-ink-900/[0.045]">
                  ...
                  <td className="px-3 py-2.5">
                    <button onClick={() => onLocate([b.lat, b.lng], 14)}
                      className="flex cursor-pointer items-center gap-1 font-mono text-[10px] text-teal-500 transition-colors hover:text-amber-600" title="Locate centroid on map">
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex justify-end gap-1">
                      {can.update && (
                        <button onClick={() => setModal({ editing: b })}
                          className="cursor-pointer rounded-[3px] border border-line-400 p-1.5 text-text-400 transition-colors hover:border-amber-600 hover:text-amber-600" title="Edit barangay">
                          <IconEdit size={13} />
                        </button>
                      )}
                      <button onClick={() => (can.del ? setDel(b) : toast("Delete requires Program Admin", "info", `signed in as ${role}`))}
                        className={`rounded-[3px] border p-1.5 transition-colors ${can.del ? "cursor-pointer border-line-400 text-text-400 hover:border-coral-500 hover:text-coral-600" : "cursor-not-allowed border-line-300 text-line-400"}`}
                        title={can.del ? "Remove barangay" : "Requires Program-Admin role"}>
                        <IconTrash size={13} />
                      </button>
                    </div>
                  </td>
      ...
      <p className="border-t border-line-300 bg-paper-200 px-4 py-2 font-mono text-[9px] tracking-[0.14em] text-text-400 uppercase">
        {can.del ? "admin role · removals enabled" : `${role} role · removals require admin`} · renaming propagates to project records
      </p>

      {modal && <BarangayForm editing={modal.editing} previewId={modal.editing?.id ?? nextKey} onClose={() => setModal(null)} />}
