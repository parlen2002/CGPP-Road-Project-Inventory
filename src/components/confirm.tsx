...
export default function ConfirmDialog({ title, sheet, message, confirmLabel, onConfirm, onClose }: {
  title: string; sheet: string; message: ReactNode; confirmLabel: string;
  onConfirm: () => void; onClose: () => void;
}) {
  return createPortal(
    <div className="anim-fade-in fixed inset-0 z-[60] grid place-items-center bg-ink-950/70 p-4" onClick={onClose}>
      <div className="anim-fade-up relative w-full max-w-md rounded-[4px] border-2 border-coral-600 bg-paper-100 shadow-2xl" onClick={(e) => e.stopPropagation()}>
