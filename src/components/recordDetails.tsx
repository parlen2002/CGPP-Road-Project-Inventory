...
export default function RecordDetails({ record }: { record: ProjectRecord }) {
  ...
  const variantBtn = (variant: SpecVariant, filled: number, label: string) => (
    <button onClick={() => setModal({ kind: "spec", variant })}
      className="group flex flex-1 cursor-pointer items-center justify-between gap-2 rounded-[3px] border border-line-400 bg-white/60 px-3 py-2 transition-all hover:border-ink-800 hover:bg-paper-200">
            ...
            return (
              <div key={kind} className="rounded-[3px] border border-line-400 bg-white/60">
                <div className="flex items-center justify-between border-b border-line-300 px-3 py-2">
                  ...
                  <button onClick={() => setModal({ kind: "order", order: kind })}
                    className="flex cursor-pointer items-center gap-1 font-mono text-[8.5px] font-bold tracking-wider text-pine-600 uppercase transition-colors hover:text-pine-500">
                    ...
                    {list.map((o) => {
                      ...
                      return (
                        <li key={o.id} className="flex items-center gap-2 px-3 py-2">
                          ...
                          <button onClick={() => (admin ? setDelOrder({ kind, id: o.id, label: o.orderNo }) : toast("Program-Admin mode required", "info", "toggle ADMIN to delete orders"))}
                            className={`shrink-0 p-1 ${admin ? "cursor-pointer text-text-400 transition-colors hover:text-coral-600" : "cursor-not-allowed text-line-400"}`}
                            title={admin ? "Delete order (admin)" : "Requires program-admin mode"}>
                            <IconTrash size={12} />
                          </button>
                        </li>
                      );
