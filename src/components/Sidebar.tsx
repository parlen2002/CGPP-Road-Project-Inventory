...
import { IconMap, IconRoad, IconLayers, IconChart, IconStack, IconParcels, IconBarangay, IconStreet, IconBoard } from "./icons";
import { cityRoads } from "../data/roads";

export type PageId = "overview" | "inventory" | "projects" | "cadastre" | "roads" | "barangays" | "analytics" | "system";

const items: { id: PageId; label: string; icon: (p: { size?: number; className?: string }) => ReactNode; note: string }[] = [
  { id: "overview", label: "Overview", icon: (p) => <IconMap {...p} />, note: "GEOSPATIAL CONSOLE" },
  { id: "inventory", label: "Road Inventory", icon: (p) => <IconRoad {...p} />, note: `${cityRoads.length} SEGMENTS` },
  ...
  { id: "roads", label: "Road Registry", icon: (p) => <IconStreet {...p} />, note: "STREET NAMING BASIS" },
  ...
  return (
    <aside className="relative z-20 flex w-[62px] shrink-0 flex-col border-r-2 border-ink-700 bg-ink-900 lg:w-60">
      <svg className="pointer-events-none absolute inset-0 h-full w-full opacity-[0.16]" preserveAspectRatio="none" viewBox="0 0 240 800">
        {[0, 1, 2, 3, 4, 5].map((i) => (
          <path key={i} d={`M-20 ${90 + i * 128} C 60 ${40 + i * 128}, 120 ${150 + i * 128}, 260 ${70 + i * 128}`} fill="none" stroke="#3f6350" strokeWidth="1" />
        ))}
      </svg>

      <nav className="relative mt-4 flex-1 space-y-1 overflow-y-auto px-2.5 lg:px-3">
        <p className="mb-3 hidden px-2 font-mono text-[9px] tracking-[0.26em] text-paper-300/40 uppercase lg:block">Modules</p>
        {items.map((it) => {
          ...
          return (
            <button key={it.id} onClick={() => onNavigate(it.id)}
              ...
              <span className="hidden min-w-0 flex-1 lg:block">
                ...
                <span className={`mt-0.5 block font-mono text-[8.5px] tracking-[0.2em] ${active ? "text-amber-400/70" : "text-paper-300/35"}`}>{it.note}</span>
              </span>
            </button>
          );
        })}
      ...
      <div className="relative border-t border-ink-700 p-2.5 lg:p-3">
        ...
        <p className="mt-2.5 flex items-center gap-1.5 text-center font-mono text-[8.5px] tracking-[0.18em] text-paper-300/30 uppercase lg:text-left">
          <IconBoard size={11} /> OCE · PPC GIS UNIT
        </p>
      </div>
    </aside>
  );
