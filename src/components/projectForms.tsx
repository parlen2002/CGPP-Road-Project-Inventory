/* Project encoder — 21-field record form. Dropdowns resolve from the Technical
   Catalogs with static fallbacks. Barangay coverage uses a searchable multi-select. */
...
export function ProjectForm({ onClose, editing }: { onClose: () => void; editing?: ProjectRecord | null }) {
  ...
  /* catalog lists with static fallbacks */
  ...
  const typeList = list("projectTypes", PROJECT_TYPES);
  const modeList = list("modes", MODES);
  const fundList = list("funds", FUNDS);
  const treatList = [...TREATMENT_ORDER]; // fixed pavement ladder — not catalog-managed
  ...
  const submit = () => {
    const base = {
      name: f.name.trim(),
      type: (typeList.includes(type) ? type : "Concreting") as ProjectType,
      mode: f.mode, fund: f.fund, objectCode, folderNo: f.folderNo || "OCE-IF-PENDING",
      ...
      treatment: (treatList.indexOf(treatment as never) >= 0 ? treatment : null) as ProjectRecord["treatment"],
  ...
  const engrOptions = engineers.map((e) => ({ value: e.id, label: e.name, sub: e.position }));
  ...
  return (
    <Modal title={editing ? "Edit Road Project" : "Encode Road Project"} sheet={`project_records · ${editing ? editing.id : nextRecordId(records)}`} onClose={onClose} wide>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <Field label="Source of fund">
          <select className={inputCls} value={f.fund} onChange={(e) => setF({ ...f, fund: e.target.value })}>
            {fundList.map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
        </Field>
        ...
        <Field label="Project in-charge (FK) *">
          <SearchSelect value={f.inchargeId} onChange={(v) => setF({ ...f, inchargeId: v as string })} options={engrOptions} placeholder="— select engineer —" invalid={editing !== null && !f.inchargeId} />
        </Field>
      ...
      <div className="mt-5 flex items-center justify-between gap-3 border-t border-line-300 pt-4">
        <p className="flex items-center gap-1.5 font-mono text-[9.5px] text-text-400 uppercase">
          <IconPlus size={0} className="hidden" />
