/* Technical catalogs manager — CRUD over every dropdown list, categorized by
   domain. Add/rename gated by the catalogs capability; delete by admin. */
...
import { IconPlus, IconCheck, IconClose, IconTrash, IconEdit, IconLock } from "./icons";

export default function CatalogManager() {
  const { catalogItems, records, roadsReg, barangays, personnel } = useStore();
  ...
  const delUsage = delItem ? itemUsage(delItem.group, delItem.value, records, roadsReg, barangays, personnel) : 0;

  const commitAdd = (g: CatalogGroup) => {
    const v = draft.trim();
    if (!v) return;
    if (byGroup.get(g.key)!.some((c) => c.value.toLowerCase() === v.toLowerCase())) {
      toast(v, "info", "already in this list");
      return;
    }
      ...
      <div key={g.key} className="rounded-[4px] border border-line-300 bg-paper-100 transition-shadow hover:shadow-[0_10px_28px_rgba(12,25,19,0.1)]">
            ...
            return (
              <li key={item.id} className="group flex items-center gap-2 px-3.5 py-2 transition-colors hover:bg-paper-200/70">
                {editId === item.id && can.catalog ? (
                  ...
                  <>
                    {item.code && (
                      <span className="shrink-0 rounded-[2px] bg-teal-500/15 px-1 py-0.5 font-mono text-[8.5px] font-bold text-teal-500" title="Stored code (hidden in dropdowns)">{item.code}</span>
                    )}
                    ...
                    {can.catalog ? (
                      ...
                      <span className="shrink-0 p-1 text-line-400 opacity-40" title="Catalog editing not available for this role"><IconLock size={12} /></span>
                    )}
                    {can.del ? (
                      <button onClick={() => setDelItem(item)}
                        className="shrink-0 cursor-pointer p-1 text-text-400 opacity-0 transition-all group-hover:opacity-100 hover:text-coral-600" title="Remove option (admin)">
                        <IconTrash size={13} />
                      </button>
                    ) : (
                      <span className="shrink-0 p-1 text-line-400 opacity-40" title="Requires Program Admin"><IconLock size={12} /></span>
                    )}
                  </>
                )}
              </li>
        ...
        <div className="border-t border-line-300 p-2.5">
          {adding === g.key ? (
            ...
            <button onClick={() => setAdding(g.key)}
            ...
            <p className="flex items-center justify-center gap-1.5 rounded-[3px] border border-dashed border-line-300 py-1.5 font-mono text-[9px] tracking-[0.14em] text-line-400 uppercase">
              <IconLock size={10} /> Editing requires catalogs access
            </p>
          )}
        </div>
      </div>
  ...
  const DOMAINS: { key: CatalogGroup["domain"]; title: string; note: string }[] = [
    ...
    { key: "personnel", title: "Personnel — Prefixes, Divisions & Positions", note: "Feed signup, accounts and the personnel board" },
  ];
