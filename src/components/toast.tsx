...
interface ToastItem { id: number; kind: ToastKind; msg: string; sub?: string; }
...
export function toast(msg: string, kind: ToastKind = "saved", sub?: string) {
  ...
  setTimeout(() => { items = items.filter((t) => t.id !== id); listeners.forEach((l) => l()); }, 3600);
}

export function Toaster() {
  useSyncExternalStore((l) => { listeners.add(l); return () => { listeners.delete(l); }; }, () => items);
  useEffect(() => {
  ...
  return (
    <div className="pointer-events-none fixed right-4 bottom-4 z-[70] flex w-[320px] flex-col gap-2">
      {items.map((t) => {
        const m = META[t.kind];
        return (
          <div key={t.id} className="anim-fade-up pointer-events-auto flex items-start gap-3 rounded-[4px] border-2 bg-ink-900 p-3 shadow-[0_16px_40px_rgba(7,17,12,0.5)]" style={{ borderColor: m.color }}>
            ...
            <button onClick={() => { items = items.filter((x) => x.id !== t.id); listeners.forEach((l) => l()); }}
              className="cursor-pointer font-mono text-[10px] text-paper-300/40 transition-colors hover:text-paper-100" aria-label="Dismiss">✕</button>
          </div>
        );
      })}
    </div>
