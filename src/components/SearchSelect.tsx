/* Searchable dropdown — plain select while short, type-to-filter once options
   exceed SEARCHABLE_AT (10). Supports single and multi (checkbox) modes. */

import { useEffect, useMemo, useRef, useState } from "react";
import { IconCheck } from "./icons";

export interface SSOption { value: string; label: string; sub?: string; disabled?: boolean; }
export const SEARCHABLE_AT = 10;
    ...
    <div ref={rootRef} className="relative">
      <button type="button" onClick={() => setOpen((o) => !o)}
        ...
        <span className="flex shrink-0 items-center gap-1.5">
          {multiple && sel.length > 0 && <span className="rounded-sm bg-pine-600 px-1.5 py-0.5 font-mono text-[9px] font-bold text-paper-100">{sel.length}</span>}
      ...
      {open && (
        <div className="anim-fade-up absolute left-0 right-0 z-30 mt-1 overflow-hidden rounded-[4px] border-2 border-ink-800 bg-paper-100 shadow-[0_18px_44px_rgba(12,25,19,0.28)]">
          ...
          <ul className="max-h-[240px] overflow-y-auto py-1">
              ...
              return (
                <li key={o.value}>
                  <button type="button" onClick={() => toggle(o.value)}
                    ...
                    <span className="w-5 shrink-0 text-right font-mono text-[8.5px] text-text-400/60 tabular">{i + 1}</span>
                    {multiple && (
            ...
            {filtered.length === 0 && <li className="px-3 py-4 text-center font-mono text-[10px] text-text-400">— no matches —</li>}
          </ul>
        </div>
      )}
    </div>
