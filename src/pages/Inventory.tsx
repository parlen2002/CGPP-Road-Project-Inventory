/* Road inventory — DERIVED from the project ledger: only roads a record is
   linked to are listed. Treatment per road is the highest pavement stage
   among its linked projects (fallback: surveyed surface). */

import { useEffect, useMemo, useState } from "react";
import { cityRoads, TREATMENT_COLOR, TREATMENT_SHORT, type Road, type Treatment } from "../data/roads";
...
export default function Inventory({ selectedId, onSelect, onLocate }: {
  ...
  /* ledger-derived rollups per registry road */
  ...
  /* only roads a project record links to */
  const linkedIds = useMemo(() => new Set(records.map((r) => r.roadId).filter((x): x is string => !!x)), [records]);
  const roads = useMemo(() => cityRoads.filter((r) => linkedIds.has(r.id)), [linkedIds]);
  ...
  /* pavement ladder from the ledger subset */
  const ladder = useMemo(() => {
    const l: Record<string, number> = { "Road Opening": 0, "Road Graveling": 0, Asphalting: 0, Concreting: 0 };
    roads.forEach((r) => { const d = derived.get(r.id); if (d) l[d.treatment] = +(l[d.treatment] + r.lengthKm).toFixed(1); });
    return l;
  }, [roads, derived]);
  const openedKm = roads.reduce((s, r) => s + r.lengthKm, 0);

  const barangays = useMemo(() => ["All", ...Array.from(new Set(roads.map((r) => r.barangay))).sort()], [roads]);
  ...
  }, [roads, search, brgy, surface, cond, sortKey, dir]);
  ...
  const exportCsv = () => {
    const head = "road_id,name,barangay,class,treatment,treatment_year,from_projects,surface,condition,length_km,width_m,lanes,aadt,pci,geom_wkt_linestring";
    const rows = filtered.map((r) => {
      const d = derived.get(r.id)!;
      return [r.id, `"${r.name}"`, `"${r.barangay}"`, r.roadClass, `"${d.treatment}"`, d.treatmentYear || "", d.byProjects ? "yes" : "no", r.surface, r.condition, r.lengthKm, r.widthM, r.lanes, r.aadt, r.pci,
  ...
  const stages: { label: Treatment; tone: string }[] = [
    { label: "Concreting", tone: TREATMENT_COLOR.Concreting },
    { label: "Asphalting", tone: TREATMENT_COLOR.Asphalting },
    { label: "Road Graveling", tone: TREATMENT_COLOR["Road Graveling"] },
    { label: "Road Opening", tone: TREATMENT_COLOR["Road Opening"] },
  ];
  ...
  return (
    <div className="mx-auto max-w-[1520px] px-4 py-6 sm:px-6">
      <PageHeader
        ...
        subtitle="Derived from the PROJECT LEDGER — only roads a project record is linked to are listed, matching the geospatial console. Each segment is accounted by its latest works: road opening, graveling, asphalting or concreting."
      />
              ...
              <div className="flex h-[14px] overflow-hidden rounded-[3px] border border-ink-600">
                {stages.map((s) => (
                  <div key={s.label} title={`${s.label} ${(ladder[s.label] ?? 0).toFixed(1)} km`}
                    style={{ width: `${openedKm ? ((ladder[s.label] ?? 0) / openedKm) * 100 : 0}%`, background: s.tone, transition: "width 0.8s cubic-bezier(0.22,1,0.36,1)" }} />
                ))}
              </div>
              <div className="mt-2.5 grid grid-cols-2 gap-x-6 gap-y-1 sm:grid-cols-4">
                {stages.map((s) => (
                  <p key={s.label} className="flex items-center gap-1.5 font-mono text-[9.5px] tracking-wider text-paper-300/75 uppercase">
                    <i className="h-2 w-2 rounded-[2px]" style={{ background: s.tone }} />
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1 px-1 font-mono text-[10.5px] tracking-wider text-text-600 uppercase">
        <span><b className="text-ink-900">{filtered.length}</b> / {roads.length} ledger-linked roads</span>
        ...
        <span className="rounded-[3px] border border-line-400 px-1.5 py-0.5 text-[9.5px] text-text-400">
          {(() => {
            const unlinked = cityRoads.length - linkedIds.size;
        ...
        <PrintPortal orientation="landscape">
          <InventorySheet
            ...
            rows={filtered.map((r) => {
              const d = derived.get(r.id)!;
              return {
                r: { id: r.id, name: r.name, barangay: r.barangay, surface: r.surface, condition: r.condition, lengthKm: r.lengthKm, widthM: r.widthM, lanes: r.lanes, aadt: r.aadt, pci: r.pci },
