/* Network analytics — growth, budget, surface & condition mix, jurisdiction. */
...
import { PROJECT_TYPES, STATUS_META, STATUS_LABELS, statusOf, typeColorOf } from "../data/registry";
  ...
  return (
    <div className="mx-auto max-w-[1520px] px-4 py-6 sm:px-6">
      <PageHeader
        ...
        subtitle="Longitudinal paving growth, budget distribution across project types, surface composition and condition exposure — aggregated nightly from the PostGIS warehouse."
      />
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-12">
        ...
        <Reveal className="lg:col-span-4" delay={90}>
          <div className="relative h-full rounded-[4px] border-2 border-ink-800 bg-ink-900 p-5">
            ...
            <Donut data={statusDonut} centerLabel={`₱${(records.reduce((s, r) => s + r.contractedAmount, 0) / 1e6).toFixed(1)}M`} centerSub="total" size={172} />
          </div>
        </Reveal>
        ...
        <Reveal className="lg:col-span-4" delay={80}>
          <div className="relative h-full rounded-[4px] border border-line-300 bg-paper-100 p-5">
            ...
            <p className="mb-4 font-mono text-[9.5px] tracking-[0.16em] text-text-400 uppercase">
              km requiring action: <b className="text-coral-600">{conditionMix.find((c) => c.label === "Poor")!.km}</b>
            </p>
