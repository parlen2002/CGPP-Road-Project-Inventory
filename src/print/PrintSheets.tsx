/* Print engine — renders sheets into #print-root and triggers window.print().
   ...
   Signatories resolve LIVE from the personnel dataset. */
...
function SheetHeader({ sheet, title, subtitle }: { sheet: string; title: string; subtitle: string }) {
  ...
  return (
    <div className="phead" style={{ border: `2px solid ${INK}`, background: "#f5f7f0" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", borderBottom: `2px solid ${INK}` }}>
        <div style={{ width: 38, height: 38, border: `2px solid ${INK}`, borderRadius: "50%", display: "grid", placeItems: "center", flexShrink: 0 }}>
          <span style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 13 }}>OCE</span>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p className="pmono" style={{ fontSize: 8.5, letterSpacing: "0.2em", textTransform: "uppercase", color: "#71826f", margin: 0 }}>
            Republic of the Philippines · City of Puerto Princesa · Office of the City Engineer
          </p>
          <p style={{ fontFamily: "var(--font-display)", fontSize: 24, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.02em", margin: 0, color: INK, lineHeight: 1.05 }}>{title}</p>
          <p className="pmono" style={{ fontSize: 9, letterSpacing: "0.08em", textTransform: "uppercase", color: "#47584d", margin: "2px 0 0" }}>{subtitle}</p>
        </div>
        <div className="pmono" style={{ textAlign: "right", fontSize: 8.5, letterSpacing: "0.14em", textTransform: "uppercase", color: "#71826f", lineHeight: 1.6 }}>
          DWG {sheet}<br />REV C · {today}
        </div>
      </div>
      <div style={{ display: "flex", height: 5 }}>
        <span style={{ flex: 3, background: AMBER }} />
        <span style={{ flex: 1.5, background: "#175c43" }} />
        <span style={{ flex: 1, background: INK }} />
        <span style={{ flex: 0.6, background: "#de5a36" }} />
      </div>
    </div>
  );
}
...
function SheetBar() {
  return (
    <div className="pmono" style={{ display: "flex", justifyContent: "space-between", fontSize: 8, letterSpacing: "0.16em", textTransform: "uppercase", color: "#71826f", margin: "6px 0 2px" }}>
      <span>RPIS · Road Project Inventory System</span>
      <span>Generated {new Date().toLocaleString("en-PH", { timeZone: "Asia/Manila" })} · WGS 84 · EPSG 4326</span>
    </div>
  );
}
...
function CertFooter() {
  ...
  const asst =
    personnel.find((p) => p.position === "Assistant City Engineer") ??
    personnel.find((p) => /assistant\s+city\s+engineer/i.test(p.position));
  const cityEngr =
    personnel.find((p) => p.position === "City Engineer") ??
    personnel.find((p) => /city\s+engineer/i.test(p.position) && !/assistant/i.test(p.position));
  const checked = asst ?? { name: "Engr. Maria L. Fernandez", position: "CGPP Assistant Department Head II (Assistant City Engineer)" };
  const approved = cityEngr ?? { name: "Engr. Ramon C. Villanueva", position: "CGPP Department Head II (City Engineer)" };
  const prepared = user ? { name: user.name, position: `${user.position} · ${user.divisionCode || user.division}` } : { name: "Registry Custodian", position: "OCE" };
  return (
    <div className="pavoid" style={{ display: "flex", marginTop: 22, paddingTop: 4 }}>
      <SigBlock role="Prepared by" name={prepared.name} position={prepared.position} />
      <SigBlock role="Checked by" name={checked.name} position={checked.position} />
      <SigBlock role="Approved by" name={approved.name} position={approved.position} />
    </div>
  );
}

/* "CGPP Department Head II (City Engineer)" → main line + parenthesised line */
function splitPosition(pos?: string): { main: string; paren?: string } {
  if (!pos || !pos.trim()) return { main: "" };
  const m = pos.trim().match(/^(.*?)\s*(\([^)]+\))\s*$/);
  if (m) return { main: m[1].trim(), paren: m[2] };
  return { main: pos.trim() };
}

function SigBlock({ role, name, position }: { role: string; name: string; position?: string }) {
  const { main, paren } = splitPosition(position);
  return (
    <div className="pavoid" style={{ flex: 1, padding: "0 10px" }}>
      <p className="pmono" style={{ fontSize: 8.5, letterSpacing: "0.16em", textTransform: "uppercase", color: "#47584d", margin: "0 0 2px", textAlign: "left" }}>{role}:</p>
      <div style={{ height: 56 }} />
      <div style={{ borderTop: `1.5px solid ${INK}` }} />
      <p style={{ fontSize: 11.5, fontWeight: 700, margin: "5px 0 0", textTransform: "uppercase", textAlign: "center" }}>{name}</p>
      {main && <p className="pmono" style={{ fontSize: 8.5, color: "#47584d", margin: "2px 0 0", textAlign: "center" }}>{main}</p>}
      {paren && <p className="pmono" style={{ fontSize: 8.5, color: "#71826f", margin: "1px 0 0", textAlign: "center" }}>{paren}</p>}
    </div>
  );
}
...
export function PrintPortal({ children, orientation = "landscape" }: {
  ...
  return createPortal(
    <>
      <style>{`@media print { @page { size: A4 ${orientation}; margin: 12mm 10mm 14mm 10mm; } }`}</style>
      {children}
    </>,
    el
  );
...
export function LedgerSheet({ rows, scopeNote, contractors, personnel }: {
  rows: ProjectRecord[];
  scopeNote: string;
  ...
  return (
    <div className="print-sheet">
      ...
      <SheetBar />
      <table className="ptable pstick" style={{ marginTop: 10 }}>
        <thead>
          <tr>
...
export function InventorySheet({ rows, scopeNote, networkKm, ladder }: {
  ...
  return (
    <div className="print-sheet">
      ...
      <SheetBar />
      <div className="pavoid pstick" style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 10, border: `1.5px solid ${INK}`, padding: "8px 12px", background: "#f5f7f0" }}>
...
export function ProjectSheet({ record, contractor, engineer, linkedRoad, mapShot }: {
  ...
  const { parcels, centerlines, personnel } = useStore();
  const { user } = useAuth();
  ...
  const cityEngr =
    personnel.find((p) => p.position === "City Engineer") ??
    personnel.find((p) => /city\s+engineer/i.test(p.position) && !/assistant/i.test(p.position));
  const checkedBy = asst ?? { name: "Engr. Maria L. Fernandez", position: "CGPP Assistant Department Head II (Assistant City Engineer)" };
  const approvedBy = cityEngr ?? { name: "Engr. Ramon C. Villanueva", position: "CGPP Department Head II (City Engineer)" };
  ...
  return (
    <div className="print-sheet print-portrait">
      ...
      {/* status band */}
      <div className="pavoid pstick" style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 12, background: INK, padding: "10px 14px" }}>
      ...
      <div className="pavoid pstick" style={{ border: `1.5px solid ${INK}`, padding: "4px 14px 8px" }}>
      ...
      <div className="pavoid pstick" style={{ display: "flex", gap: 10 }}>
        ...
        <div style={{ flex: 1, border: `1.5px solid ${INK}`, padding: "8px 12px" }}>
          ...
          {engineer ? (<>
            ...
            <p className="pmono" style={{ fontSize: 10, color: "#47584d", margin: "3px 0 0" }}>{engineer.position} · {engineer.divisionCode || engineer.division}</p>
      ...
      <div className="pavoid pstick" style={{ border: `1.5px solid ${INK}`, borderTop: "none", padding: "4px 14px 6px" }}>
      ...
      <div className="pavoid pstick" style={{ border: `1.5px solid ${INK}`, padding: "8px 14px" }}>
      ...
      {techNo && specRows.length > 0 && (
        <>
          ...
          {record.revision && (
            <p className="pmono pavoid" style={{ fontSize: 9.5, color: "#47584d", margin: "6px 0 0" }}>
              Revision {record.revision.meta.revisionNo} · {fmtDate(record.revision.meta.date)} — {record.revision.meta.reason || "no reason recorded"}
            </p>
          )}
          {record.actual && (
            <p className="pmono pavoid" style={{ fontSize: 9.5, color: "#47584d", margin: "3px 0 0" }}>
              As-built certified {fmtDate(record.actual.meta.date)} by {record.actual.meta.certifiedBy || "—"} · {filledCount(act)} items
            </p>
          )}
        </>
      )}
      ...
      {rowNo && row && axis && (
        <>
          ...
          <div className="pavoid pstick" style={{ display: "flex", gap: 10, marginBottom: 8 }}>
            ...
            <Money label="Corridor area" value={fmtAreaLocal(row.corridorAreaM2)} sub={`axis ${axis.id} · ±${axis.radiusM} m`} />
            <Money label="Private affected" value={fmtAreaLocal(row.privM2)} tone="#b84423" sub="subject to acquisition" />
              ...
              <tbody>
                {row.rows.map((x) => (
                  <tr key={x.lotId}>
                    ...
                    <td className="r pmono" style={{ fontWeight: 700 }}>{fmtAreaLocal(x.affectedM2)}</td>
      ...
      {ordNo && (record.variations.length > 0 || record.suspensions.length > 0) && (
        <>
          ...
          {record.variations.length > 0 && (
            <table className="ptable pstick" style={{ marginBottom: 8 }}>
      ...
      {images.length === 0 ? (
        <p className="pavoid pstick" style={{ fontSize: 11, color: "#71826f", border: "1px dashed #cbd4c2", padding: "8px 12px", margin: 0 }}>
          No geotagged image attachments on this record. PDF supporting documents are excluded from the printed copy.
        </p>
      ) : (
        <div className="pavoid pstick" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
          {images.map((a) => (
      ...
      <div className="pavoid pstick" style={{ border: `1.5px solid ${INK}`, background: "#f5f7f0", padding: "8px 14px", minHeight: 40 }}>
        <p style={{ fontSize: 12, margin: 0, lineHeight: 1.55, whiteSpace: "pre-wrap" }}>{record.notes?.trim() || "—"}</p>
      </div>

      <div className="pavoid" style={{ display: "flex", marginTop: 22, paddingTop: 4 }}>
        <SigBlock role="Prepared by" name={user?.name ?? engineer?.name ?? "Project In-Charge"} position={user ? `${user.position} · ${user.divisionCode || user.division}` : engineer?.position} />
        <SigBlock role="Checked by" name={checkedBy.name} position={checkedBy.position} />
        <SigBlock role="Approved by" name={approvedBy.name} position={approvedBy.position} />
      </div>
    </div>
  );
}
...
function FragmentRow({ head, children }: { head: string | null; children: ReactNode }) {
  return (
    <>
      {head && (
        <tr className="phead" style={{ background: "#edf0e6" }}>
...
function fmtAreaLocal(m2: number): string {
  return m2 >= 10000 ? `${(m2 / 10000).toLocaleString("en-PH", { maximumFractionDigits: 2 })} ha`
